<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <link rel="stylesheet" href="{{asset('/frontend/dashboard/css/main.css')}}">
   <title>Dashboard</title>
</head>
<body>

    <!-- Site main start -->
   <main class="site-main">
      <div class="statistics">
         <!-- Hero section start -->
         <section class="progress">
            <div class="container">
               <div class="progress__inner">
                  <ul class="progress__list">
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-visitors">
                           <span class="number">4 382</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Посещений сайта innoweek</p>
                        </div>
                     </li>
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-views">
                           <span class="number">2 972</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Просмотров виртуального тура</p>
                        </div>
                     </li>
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-companys">
                           <span class="number">72</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Учавствующих компаний</p>
                        </div>
                     </li>
                  </ul>

                  <div class="progressing">
                     <div class="text-wrapper">
                        <p class="text progressing__heading">Топ-5 по кол-ву участников выставки по странам</p>
                     </div>
                     <div class="progressing-box">
                        <div class="progressing__ball">
                           <div class="canvas">
                              <svg class="chart" width="500" height="500" viewBox="0 0 50 50">
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                                 <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              </svg>

                              <div class="diagram-desc">
                                 <span class="number">23</span>
                                 <p class="progressing__text">Стран участников</p>
                              </div>

                              <div class="legend">
                                 <ul class="caption-list">
                                    <li class="caption-item">
                                       <span class="number progressing__item-span">87</span>
                                       <p class="text">Узбекистан</p>
                                    </li>
                                    <li class="caption-item">
                                       <span class="number progressing__item-span">10</span>
                                       <p class="text">Турция</p>
                                    </li>
                                    <li class="caption-item">
                                       <span class="number progressing__item-span">6</span>
                                       <p class="text">Россия</p>
                                    </li>
                                    <li class="caption-item">
                                       <span class="number progressing__item-span">5</span>
                                       <p class="text">Татарстан</p>
                                    </li>
                                    <li class="caption-item">
                                       <span class="number progressing__item-span">4</span>
                                       <p class="text">Азербайджан</p>
                                    </li>
                                 </ul>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </section>
         <!-- Hero section finish -->

         <!-- Section progress visitors start -->
         <section class="progress-visitors">
            <div class="container">
               <div class="progress-visitors__inner">
                  <div class="progressing">
                     <div class="text-wrapper">
                        <p class="text progressing__heading">Топ-5 по кол-ву посетителей выставки по странам</p>
                     </div>
                     <div class="progressing-box">
                        <div class="canvas">
                           <svg class="chart" width="500" height="500" viewBox="0 0 50 50">
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                              <circle class="unit" r="15.9" cx="50%" cy="50%"></circle>
                           </svg>

                           <div class="diagram-desc__end">
                              <span class="number">1 587</span>
                              <p class="progressing__text">Посетителей виртуального тура</p>
                           </div>

                           <div class="legend">
                              <ul class="caption-list">
                                 <li class="caption-item">
                                    <span class="number progressing__item-span">1 524</span>
                                    <p class="text">Узб</p>
                                 </li>
                                 <li class="caption-item">
                                    <span class="number progressing__item-span">7</span>
                                    <p class="text">Рф</p>
                                 </li>
                                 <li class="caption-item">
                                    <span class="number progressing__item-span">3</span>
                                    <p class="text">Каз</p>
                                 </li>
                                 <li class="caption-item">
                                    <span class="number progressing__item-span">2</span>
                                    <p class="text">Азе</p>
                                 </li>
                                 <li class="caption-item">
                                    <span class="number progressing__item-span">2</span>
                                    <p class="text">Рб</p>
                                 </li>
                              </ul>
                           </div>
                        </div>
                     </div>
                  </div>

                  <ul class="progress__list">
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-visitors">
                           <span class="number">0</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Посетителей выставки</p>
                        </div>
                     </li>
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-visitors-age">
                           <span class="number">29</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Средний возраст</p>
                        </div>
                     </li>
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-visitors-country">
                           <span class="number">30</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Стран посетителей</p>
                        </div>
                     </li>
                  </ul>
               </div>
            </div>
         </section>
         <!-- Section progress visitors finish -->

         <!-- Section last progress start -->
         <section class="progress">
            <div class="container">
               <div class="progress__inner">
                  <ul class="progress__list">
                     <li class="progress__item">
                        <div class="progress__item-start progress__item-amount">
                           <span class="number">3</span>
                        </div>
                        <div class="text-wrapper">
                           <p class="text">Кол-во инвестиционных соглашений</p>
                        </div>
                     </li>
                     <li class="progress__item">
                        <div class="text-wrapper">
                           <p class="text">Сумма инвестиционных соглашений</p>
                        </div>
                     </li>
                  </ul>

                  <div class="progressing">
                     <div class="text-wrapper">
                        <p class="text progressing__heading">Количество посетителей выставки в разрыве языков</p>
                     </div>
                     <div class="last-progressing">
                        <ul class="last-progressing__list">
                           <li class="last-progressing__item">
                              <span class="last-progressing__diagram"></span>
                              <span class="number">1 139</span>
                           </li>
                           <li class="last-progressing__item">
                              <span class="last-progressing__diagram"></span>
                              <span class="number">435</span>
                           </li>
                           <li class="last-progressing__item">
                              <span class="last-progressing__diagram"></span>
                              <span class="number">33</span>
                           </li>
                        </ul>
                     </div>
                  </div>
               </div>
            </div>
         </section>
         <!-- Section last progress finish -->
      </div>
      <div class="live-stream">
         <h2 class="live-stream-title">Live stream</h2>
         <ul class="live-stream-list">
            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Forum</p>
            </li>

            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Yarmarka</p>
            </li>

            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Forum-2</p>
            </li>

            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Yarmarka-2</p>
            </li>

            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Forum-3</p>
            </li>

            <li class="live-stream-item">
               <div class="live-stream-box">

               </div>
               <p class="live-stream-box-text">Yarmarka-3</p>
            </li>
         </ul>
      </div>

   </main>
   <!-- Site main finish -->

   <script src="{{asset('/frontend/dashboard/js/main.js')}}"></script>
</body>
</html>